...
//加载中的提示语
@property (strong, nonatomic) UILabel *loadingTipLable;
...
-(instancetype) initWithView:(UIView *)view
{
    ...
    if (self)
    {
        ...
        _loadingTipLable = [[UILabel alloc] init];
        _loadingTipLable.text = @"正在为首次登录EasyGoing配置数据...";
        _loadingTipLable.textColor = [UIColor whiteColor];
        _loadingTipLable.font = [UIFont systemFontOfSize:16];
        _loadingTipLable.numberOfLines = 0;
        _loadingTipLable.textAlignment = NSTextAlignmentCenter;
        _loadingTipLable.frame = CGRectMake(0, 0, view.bounds.size.width, 50);
        _loadingTipLable.center = CGPointMake(view.bounds.size.width/2.0, view.bounds.size.height/2.0);
        [self addSubview:_loadingTipLable];
    }
    return self;
}
-(void) initCommon
{
    ...
    self.backgroundColor = [[UIColor alloc] initWithRed:35/255.0 green:187/255.0 blue:119/255.0 alpha:1.0];
    //[UIColor colorWithHexCode:@"#DB7769"];
...
-(void) initContainer
{
    ...
    _containerLayer.position = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2 - 2*height);
