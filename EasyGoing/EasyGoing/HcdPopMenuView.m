...
//添加一个传入代理的方法
+ (void)createPopmenuItems:(NSArray *)items closeImageName:(NSString *)closeImageName backgroundImageUrl:(NSString *)urlStr tipStr:(NSString *)tipStr completionBlock:(selectCompletionBlock)block delegate:(UIViewController<HcdPopMenuViewDelegate> *)delegate
{
...
- (void)clickButton:(UIButton *)btn{
    if ([self.delegate respondsToSelector:@selector(didClickBtn:)]) {
        [self.delegate didClickBtn:(int)btn.tag];
    }
}
